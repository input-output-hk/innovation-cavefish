...
\fun{mkToSpec}~(l , i) &=& \fun{mkToSpec'}~l~i~\fun{initTx}
\end{array}
\end{displaymath}
...
\begin{displaymath}
\begin{array}{rlll}
  \TxAbs &=&(\inputs: \type{Set}~{\TxIn},\\
  & &\ \fun{validityInterval}: \Interval{\Tick},\\
  & &\ \mint: \Value,\\
  & &\ \fun{fee}: \N \\
  & &\ \fun{sigKeys}: \fun{Set}~\pubkey) \\
  & &\ \text{Abstract transaction type}
  \nextdef 
  \fun{mkAbs} &:& \Tx \to \TxAbs \\
  \fun{mkAbs}~\var{tx} &=& \var{tx} ~\{ ~\inputs = \var{tx} . \inputs,\\
  & &\ \fun{validityInterval}  = \var{tx} . \fun{validityInterval},\\
  & &\ \mint  = \var{tx} . \mint ,\\
  & &\ \fun{fee}  = \var{tx} . \fun{fee} \\
  & &\ \fun{sigKeys}  = \fun{dom}~(\var{tx} . \sigs)~\} \\
  & &\ \text{Abstract transaction constructor}
\end{array}
\end{displaymath}
\caption{Abstract transaction type and constructor}
\label{fig:abs-tx}
\end{figure}