...
\begin{tikzpicture}
	...
	\matrix (m)[matrix of nodes, column  sep=1.5cm,row  sep=7mm, nodes={draw=none, anchor=center,text depth=0pt} ]{
		...
		% sign abstract transaction\\[-7mm]
		& $\var{R}$\\[-7mm]
		& & Produce challenge $c=H(R,X,\var{tx}{\color{blue}||\var{nt}})$,\\[-7mm]
		& & and proof $\pi$ : $\fun{chkSpec}(\var{int_{post}},\var{tx}) = 1\ \land$\\[-7mm]
	 	& & \quad\quad\quad $\var{tx}_A = \var{Com}(\var{tx}{\color{blue}||\var{nt}};\rho)\ \land$ \\[-7mm]
		& & \quad\quad\quad $c=H(R,X,\var{tx}{\color{blue}||\var{nt}})$ \\[-7mm]
		& $(c,\pi)$ & \\[-7mm]
		% %& ZK.proof $p$ of $P (h,\  \var{tx}_A) = \true$ & \\
		 Check proof $\pi$ \\[-7mm]
		Produce  $s=r+cx$ \\[-7mm]
	...
	\draw[shorten <=-1cm,shorten >=-1cm,-latex] (m-16-2.south west)--(m-16-2.south east);
	%\draw (m-10-2.north west) rectangle (m-19-2.south east);
\end{tikzpicture}
\subsection{Security}
