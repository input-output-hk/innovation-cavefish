...
We assume that a \emph{full node} is one that is able to, given some verified state $s_0$ (e.g. a genesis state, 
or a verified checkpoint state), and a list of blocks $[b_0, ..., b_k]$ that have been disseminated across the 
network since the time of $s_0$, is able to compute the current state $s_{k-1}$ by applying 
them in sequence, i.e. computing $s_{i+1} = \fun{updateState}~(s_i, b_i)$ for $0 \leq i < k$,
and is able to determine if any of the blocks in the list are invalid. If all the 
blocks result in valid updates (i.e. $\fun{checkBlock}~(s_i, b_i) = \true$), we say 
