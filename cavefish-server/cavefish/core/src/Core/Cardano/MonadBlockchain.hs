...
module Core.Cardano.MonadBlockchain (
  -- * Monad blockchain
  MonadBlockchain (..),
  trySendTx,
  singleUTxO,
...
import Control.Lens (Prism', lens)
...
data CardanoNodeEnv = CardanoNodeEnv
  { localNodeConnectInfo :: C.LocalNodeConnectInfo
  , logger :: Logger
  }

instance HasLogger CardanoNodeEnv where
...
newtype MonadBlockchainCardanoNodeT era m a = MonadBlockchainCardanoNodeT
  { unMonadBlockchainCardanoNodeT :: ReaderT CardanoNodeEnv m a
  }
  deriving newtype
    ( Functor
    , Applicative
  ...
  deriving (MonadLogger, MonadLoggerIO) via (WithLogger CardanoNodeEnv m)

-- | Run the MonadBlockchainCardanoNodeT with the given local node connection info
runMonadBlockchainCardanoNodeT ::
  CardanoNodeEnv -> MonadBlockchainCardanoNodeT era m a -> m a
runMonadBlockchainCardanoNodeT env (MonadBlockchainCardanoNodeT action) =
...
class Monad m => MonadBlockchain era m | m -> era where
  ...
  default sendTx ::
    ...
    C.Tx era -> m (Either (ValidationError era) C.TxId)
  sendTx = lift . sendTx
  ...
  default queryProtocolParameters ::
    (MonadTrans t, m ~ t n, MonadBlockchain era n) => m (C.LedgerProtocolParameters era)
  ...
  default querySlotNo ::
    (MonadTrans t, m ~ t n, MonadBlockchain era n) => m (C.SlotNo, SlotLength, UTCTime)
...
instance
  (MonadIO m, MonadLogger m, C.IsShelleyBasedEra era) =>
  MonadBlockchain era (MonadBlockchainCardanoNodeT era m)
  where
  sendTx tx = MonadBlockchainCardanoNodeT $ do
    let txId = C.getTxId (C.getTxBody tx)
    logDebug $ "Submitting transaction-id to node" :# ["txId" .= txId]
    info <- asks localNodeConnectInfo
  ...
  utxoByTxIn txIns =
    runQuery'
      (C.QueryInEra (C.QueryInShelleyBasedEra C.shelleyBasedEra (C.QueryUTxO (C.QueryUTxOByTxIn txIns))))

  queryProtocolParameters =
    C.LedgerProtocolParameters
      <$> runQuery' (C.QueryInEra (C.QueryInShelleyBasedEra C.shelleyBasedEra C.QueryProtocolParameters))
  ...
  queryNetworkId = MonadBlockchainCardanoNodeT $ asks (C.localNodeNetworkId . localNodeConnectInfo)
...
runQuery' ::
  (MonadIO m, Show e1) => C.QueryInMode (Either e1 a) -> MonadBlockchainCardanoNodeT era m a
runQuery' qry =
  runQuery qry >>= \case
