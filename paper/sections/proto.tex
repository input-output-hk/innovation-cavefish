...
\label{sec:protocols}
We give an overview on our light client protocol and its components. Our protocol maximizes space and communication efficiency by essentially pushing all storage and lookup tasks to the SP, whilst guaranteeing the resulting transaction is safe for the light client to sign, and compensating their SP for their effort. This is non-trivial as the transaction must be signed sight-unseen (blinded) otherwise, the light client could simply remove the payment to the SP and submit the modified transaction.

To satisfy both parties, we rely on cryptographic tools as well as some established features in implemented blockchains such as transaction validity periods and handling of invalid transactions. In the rest of this section, we make the following assumption: all communication between a light client and a service provider happens over a secure channel (e.g. a TLS connection).

\subsection{Definitions and requirements}

\begin{definition}[Correctness] A transaction building protocol is \textbf{correct} if for an honest light client, and service provider and any state $\LState_i \in \LState$ and intent $\var{int_{post}} \in \mathcal{I}_\mathsf{post}$ such that: if $\fun{mkToSpec}(\LState_i, \var{int_{post}}) \neq \bot$, then the protocol completes and the transaction $\Tx'$ produced by the SP is such that $\fun{\fun{checkTxL}~}(\LState_i, \Tx')=1$. \end{definition}

We use ${\fun{checkTxL}~}(l, \tx)$  as shorthand for  $\fun{checkTx}~((\var{slot}, \var{fee}), \var{utxo},~\tx)$, with ledger state $l$ 
...
, then it must be that $\fun{chkSpec}(\var{int_{post}} , \Tx')=1$. \end{definition}
...
For an honest service provider SP and any client $C_\mathcal{A}$ we have that: client provides 2 states $\LState_0, \LState_1$ and one intent $\var{int_{post}} \in \mathcal{I}_\mathsf{post}$ such that for $i=0,1$ it is $ \fun{mkToSpec}(\LState_i, \var{int_{post}}) \neq \bot$.  The experiment flips a coin $d$ and runs the protocol between SP and C* using state $\LState_d$. $C_\mathcal{A}$ outputs $d^*$. The adversarial client wins iff $d=d^*$. \end{definition}
...
\subsection{Cavefish: a communication-optimal transaction building protocol}
...
\noindent\textbf{Transaction Expiry.} For safety, we require that transactions can be set to be valid only within a certain time window. This is to prevent trivial attacks where a malicious SP delays posting a transaction indefinitely. The user could contact a different SP to create a second transaction with similar parameters (e.g. by paying the same recipient with different coins) with the risk that the first SP would also post their transaction, making the user pay twice.
...
\noindent\textbf{Trusted Setup.} We specify a proof system using a trapdoor, which is a common characteristic of efficient options such as Groth16 \cite{cryptoeprint:2016/260} or Plonk \cite{plonk}. It is possible to use alternatives featuring transparent setup with some impact to communication size. In the case of a transparent setup for $\NArg$, we still need to generate keys for $\PKE$. In the case of Elgamal in the random oracle model, this can be accomplished by hashing an unpredictable string into a group element.  

\subsection{Protocol description}
\label{sec:mainproto}
The protocol operates as follows: the light client connects to the SP and sends a posting intent $\var{int_{post}}$, as well as a public key that has received payments (and, optionally a BIP-032 chaincode so that the SP can also derive child keys from the initial one --we explain the details in Section \ref{sec:xclient}). The SP will then search for UTXOs belonging to the client and form a transaction that satisfies the intent $\var{int_{post}}$, including in the transaction a tip for the SP's own work. Lastly, the SP adds a random note $\nt$ to the note field of the transaction. Then, the client and SP engage in a weakly blind predicate signature protocol so as to efficiently have the client sign the transaction whilst  ensuring that (1) the client needs to sign before learning the full transaction (typically the used UTXOs are hidden), and (2) that the signed transaction satisfies $\var{int_{post}}$ by means of satisfying the corresponding predicate.
...
%    \[ \tx = \fun{mkToSpec}~(\var{ls},~\var{int_{post}},~\tx_t)~\{~\fun{auxData}~=~\fun{random}~\} \]
...
%    \[ \chkSpec{(\var{int_{post}})}(\TxAbs )= \true \]
%    \item[(viii)] Proof Check. Blind signatures are computed by (see Section \ref{sec:blind} for details), with $\var{sks} = \fun{getSKs}~\fun{dom}~(\tx . \fun{sigs})$ :
%    \[ \var{sigs} = \{~(s,~\fun{blindSign}~(h,~\TxAbs,~ s)) \vert s \in sk_{\mathrm{client}} \cup ~\var{sks}~\} \]
%    \item[(ix)] Signatures are computed from blind signatures (see Section \ref{sec:blind} for details) :
%    \[ \var{sigs'} = \{~(s,~\fun{mkRealSig}~(s,~h,~\TxAbs,~k)) ~\vert~(s, k) \in \var{sigs}~\} \]
%    \item[(ix)] Signatures are added to the transaction
%    \[ \var{tx'} = \tx~\{~sig = \var{sigs'}~\} \]
%\end{itemize}
...
A practical example of such intents is $int_{\text{light}}(\fun{tx}):=\{\textbf{return } (\fun{makeAbs}(\tx)=\fun{makeAbs}(\var{tx_0}))\}$ i.e. the light client  fully determines the requested transaction (including SP fee) to be $\fun{tx}_0$, apart from the input UTXOs (which it does not know) and the note field $\textsf{aux}$ which is free to the SP to set. This implies that $\TxAbs$ provides no new information to the light client. This reduces the computational effort by SP invested in running the $\NArg$ prover as $\fun{chkSpec}$ consists only of substring equality checks. We note here that no range check is necessary for values: $\fun{makeAbs}$  (Section \ref{sec:dsl}) retains the prescribed value assigned to all outputs except the first (which is used as a ``change'' address), i.e., the only option for the SP is to assign all excess value to the change address.

Alternatively, in the extended variant, we set $\fun{makeAbs}(x):=\{\textbf{return } 1\}$, and change the predicate check from     $\fun{chkSpec}~(\mathcal{I}_\mathsf{post} \times \TxAbs) \to \B$ to $\fun{chkSpec}~(\mathcal{I}_\mathsf{post} \times \Tx) \to \B$. This too ensures that no information is leaked by $\TxAbs$. Our results hold for both variants.


We show our complete light client protocol  in Figure~\ref{fig:protocol-short}, leveraging the  weakly blind signatures predicate of section \ref{sect:ourscheme}. For simplicity, the figure describes the process for a single UTXO signature, if more are needed, multiple \WBPS sessions can be run sharing the same $com_\tx$.
...
\begin{tikzpicture}[every node/.style={font=\small}]
	...
	\matrix (m)[matrix of nodes, column  sep=0.1cm,row sep=0mm,
			...
			minimum height=3mm},
			column 1/.style={nodes={minimum width=4.5cm}}, 
			column 3/.style={nodes={minimum width=6.2cm}}, 
			column 2/.style={nodes={minimum width=2.6cm}} 
			 ]{
    ...
    & & Obtain newest $\LState_t\ ls$\\
		...
		& \node[rightArrowBelow] {$\var{int_{post}}, vk_c$};  \\
    		& & Determine client UTXOs from $(\var{int_{post}}, vk_c)$\\
		& & Generate $\tx = \fun{mkToSpec}(\var{ls},\var{int_{post}})$,\\
    & & $\TxAbs = \fun{mkAbs}(\tx)$, $\nt \xleftarrow{\$} \{ 0,1 \}^\notelength$ \\
    & &  Produce commitment  $com_\tx$ \\ % = $\var{Com}(\tx{\color{blue}||\var{nt}};\rho)$\\
		& \node[leftArrowBelow] {$com_\tx,\TxAbs$}; & \\
		%\ h=\fun{txid}~\tx
		% %& ZK.proof $p$ of $P (h,\  \TxAbs) = \true$ & \\
		Produce blind sig. com. $\var{R}=g^r$ &  \node[rightArrowBelow] {$\var{R}$};\\
%    & & $c=H(R,X_c,\tx{\color{blue}||\var{nt}})$ and\\
%		& & proof $\pi$ : $\fun{chkSpec}(\var{int_{post}},\tx) = 1\ \land$\\
%	 	& & \quad\quad\quad $com_\tx = \var{Com}(\tx{\color{blue}||\var{nt}};\rho)\ \land$\\
%		& & \quad\quad\quad $c=H(R,X_c,\tx{\color{blue}||\var{nt}})$ \\
		&  \node[leftArrowBelow] {$c,\pi$}; & Produce challenge $c$ and proof $\pi$ \\
		% %& ZK.proof $p$ of $P (h,\  \TxAbs) = \true$ & \\
		 Check proof $\pi$ \\
		Produce  $s=r+cx$ & \node[rightArrowBelow] {$\var{s}$}; & Produce signature $\sigma = (R,s)$\\
		& & If valid, post $(\tx||\var{aux},\sigma)$ on chain.\\
	};
	\draw[thick] (m-1-1.south east)--(m-1-1.south west);
	\draw[thick] (m-1-3.south east)--(m-1-3.south west);
	\node[fit={(m-7-3.south west) (m-13-3.south east) (m-10-1.north west)},
   ultra thick, dotted, inner xsep=11pt, inner ysep=0pt,
    rounded corners=1mm, draw=gray, label={[gray,anchor=north west]north west:WBPS Execution for $m:=\tx||\nt $}]{};	
\end{tikzpicture}
\caption{Cavefish protocol featuring WBPS scheme. A detailed descripton of the WBPS scheme is in Fig.~\ref{fig:WBSP}.}
...
%		& & $\fun{chkSpec}(\var{int_{post}},\TxAbs) = 1$ \\[-7mm]
%		& $(c,\pi)$ & \\[-7mm]
%		% %& ZK.proof $p$ of $P (h,\  \TxAbs) = \true$ & \\
...
\noindent\textbf{Liveness against Malicious SPs.}
...
$\fun{opt}~\tx = \fun{coinValue}~(\sum_{o \in~\var{txd}.\fun{spentOuts}} \var{o}.\val)$.
A transaction $\tx$ is \emph{optimal} in a set $S \in \type{Set}~\TxAbs$ when
$\fun{opt}~\tx = \fun{min}~\{~\fun{opt}~\var{tx'}~\vert~\var{tx'}~\in~S~\}$.

To get a multi-SP protocol, the first step of the single-SP protocol in Figure~\ref{fig:protocol-short}
must be augmented, so that the pair $(\fun{opt}, \var{int_{post}}$ is sent to each SP instead of just
sending $\var{int_{post}}$.
Upon receiving transactions $\tx_{A,i}$, with $0\leq i < k$, from each of the $k$ SPs responding
to LC's query, LC will engage in the rest of the protocol only with the sender of $\tx_{A,i}$.
