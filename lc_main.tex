...
\begin{tikzpicture}[every node/.style={font=\small}]
	...
	\matrix (m)[matrix of nodes, column  sep=0.1cm,row sep=0mm,
			...
			minimum height=3mm},
			column 1/.style={nodes={minimum width=5cm}}, 
			column 3/.style={nodes={minimum width=5cm}}, 
			column 2/.style={nodes={minimum width=2.0cm}} 
			 ]{
		\textbf{Signer (LC)} & & \textbf{User (SP)}\\
		Generate intent $\var{int_{post}}$ \\ % \in \mathcal{I}_\mathsf{post}$, \\
		& \node[rightArrowBelow] {$(\var{int_{post}}, vk_{\mathrm{client}})$};  \\
		& & Determine required verif. key(s) $\var{X}$ \\
		& & from $\var{int_{post}}, vk_{\mathrm{client}}$\\
		& & Generate transaction $\var{tx}$, {\color{blue}note $\var{nt}$}, \\
		&&  and $\var{tx}_A$ = $\var{Com}(\var{tx}{\color{blue}||\var{nt}};\rho)$ \\
		& \node[leftArrowBelow] {$\var{tx}_A$}; & \\
		%\ h=\fun{txid}~\var{tx}
		% %& ZK.proof $p$ of $P (h,\  \var{tx}_A) = \true$ & \\
		Produce Blind.  \\
			Sig. Commitment $\var{R}=g^r$ \\
		% sign abstract transaction\\[-7mm]
		&  \node[rightArrowBelow] {$\var{R}$};\\
		& & Produce challenge $c=H(R,X,\var{tx}{\color{blue}||\var{nt}})$,\\
		& & and proof $\pi$ : $\fun{chkSpec}(\var{int_{post}},\var{tx}) = 1\ \land$\\
	 	& & \quad\quad\quad $\var{tx}_A = \var{Com}(\var{tx}{\color{blue}||\var{nt}};\rho)\ \land$\\
		& & \quad\quad\quad $c=H(R,X,\var{tx}{\color{blue}||\var{nt}})$ \\
		...
		& & Produce signature $\sigma$\\
		& & from s, where $\sigma = (R,s)$.\\
	...
	\node[fit={(m-7-3.north west) (m-22-3.south east) (m-9-1.north west)},
      ultra thick, dotted, inner xsep=15pt, inner ysep=0pt, rounded corners=1mm, draw=gray, label={[gray,align=center]270:WBPS Execution}]{};	
\end{tikzpicture}
\subsection{Security}
...
\mathrm{R}_{\mathrm{WBPS}}
	(\underbrace{(q, \mathbb{G}, G, \mathsf{H})}_{\text{public parameters}\ par},\
	\overbrace{(X, R, \var{tx}_A, \fun{chkSpec}(\var{int_{post}}, \cdot ))}^{\text{known information}\ \theta}\ ,\
