...
template CardanoWBPS(n) {
    ...
    component hash = Sha512_hash_bits_digest(n*bits+256+256);
    for (i=0; i<256; i+=8) {
        for(j=0; j<8; j++) {
    ...
    component msgToBits[n];
    for (i=0; i<n; i++) {
        msgToBits[i] = Num2Bits(bits); 
        msg[i] ==> msgToBits[i].in;
        for(j=0; j<bits; j++) {
            msgToBits[i].out[j] ==> hash.inp_bits[512+i*bits+j];
        }
    }
...
component main {public [A,R8]}= CardanoWBPS(8);
