...
module WBPS.Core.Session.Steps.Demonstration.Artefacts.Commitment.Build (
  build,
  Context (..),
  Input (..),
) where
...
import WBPS.Adapter.CLI.Wrapped.Circom (
...
import WBPS.Core.Session.Steps.Demonstration.Artefacts.PreparedMessage (MessageBits)
import WBPS.Core.Setup.Circuit.FileScheme (
  Account (session, shellLogs),
  BuildCommitmentInternals (BuildCommitmentInternals, input, output, statementOutput),
...
getGenerateBuildCommitmentWitnessProcess accountDirectory =
  do
    setup <- asks (buildCommitment . setup)
    internals <- asks (buildCommitmentInternals . demonstration . session . account)
    return $
